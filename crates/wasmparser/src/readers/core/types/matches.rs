...
use crate::{
    types::{CoreTypeId, RecGroupId, TypeList}, ArrayType, CompositeType, FieldType, FuncType, IndexedFuncType, RefType, StorageType, StructType, SubType, ValType
};
...
impl<'a> Matches for WithRecGroup<&'a IndexedFuncType> {
    fn matches(types: &TypeList, a: Self, b: Self) -> bool {
        if a.params().len() != b.params().len() || a.results().len() != b.results().len() {
            return false;
        }

        // TODO: Deal with contrains (or, better yet, just say no subtyping on indexed func types)
